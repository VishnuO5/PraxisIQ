...
if page == "Overview":
    ...
    st.markdown("<hr/>", unsafe_allow_html=True)
    finding(
        "Why a dental clinic dataset for Trust &amp; Safety, on purpose",
        "This domain was picked because it has no surface-level overlap with content "
        "platforms — no comments section, no uploads, nothing that already looks like "
        "the target problem. The workflows built here — burst detection, repeat-actor "
        "flagging, severity scoring, appeals, capacity planning — are the same analytical "
        "pattern regardless of what's being moderated. Proving that pattern transfers to an "
        "unrelated domain is a stronger signal than building something that was already "
        "shaped like the answer."
    )
    section("Skill Alignment", "Platform Trust &amp; Safety Engineering")
    cols = st.columns(3)
    alignments = [
    ...
    if st.button("⬇  Generate Executive Report PDF", type="primary", width='content'):
        # Gather live data
...
# ─────────────────────────────────────────────
    ...
    finding(
        ...
        f"A moderation queue without a capacity model is just a list. At platform scale, "
...
# ─────────────────────────────────────────────
    ...
    st.markdown("<hr/>", unsafe_allow_html=True)
    section("What I Would Do Differently at Platform Scale", "Limitations & Production Gap")
    scale_gaps = [
        (
            "Streaming ingestion over batch processing",
            "This pipeline runs as a batch job on static data. At platform scale, reviews arrive continuously. "
    ...
    defences = [
        (
            ...
            f"the algorithm can't distinguish motive, only pattern. That's true at platform scale too. "
        ...
        (
            ...
            f"What changes at platform scale is throughput (millions of items/day vs 300 reviews), "
        ...
        (
            ...
            f"Whether you're labeling 300 dental reviews or 3 million platform comments, "
            ...
            f"documented here is the same limitation that drives large-scale labeler training programs — "
        ...
        (
            ...
            f"is a patient safety signal. On a large content platform, the equivalent is content that causes real-world harm "
...
# ─────────────────────────────────────────────
    ...
    finding(
        ...
        f"At platform scale, these playbooks live in internal wikis and are drilled with tabletop exercises — "
...
# ─────────────────────────────────────────────
# ─────────────────────────────────────────────
elif page == "Enforcement & Ops":
    page_header(
        "Account Enforcement · Appeals · Capacity",
        "Enforcement & Operations",
        "Account-level enforcement actions, the appeals lifecycle, abuse vector taxonomy, and staffing/SLA capacity planning"
    )

    ENFORCEMENT_SUBPAGES = [
        "Account Enforcement", "Appeals & Reinstatement", "Abuse Vector Taxonomy",
        "Ops Capacity", "Real-Time Simulator",
    ]
    if "enforcement_subpage" not in st.session_state:
        st.session_state["enforcement_subpage"] = ENFORCEMENT_SUBPAGES[0]

    st.caption("Click a section below to switch views")
    sub_cols = st.columns(len(ENFORCEMENT_SUBPAGES))
    for sub_col, sub_name in zip(sub_cols, ENFORCEMENT_SUBPAGES):
        is_active = st.session_state["enforcement_subpage"] == sub_name
        with sub_col:
            if st.button(
                sub_name,
                key=f"enf_subnav_{sub_name}",
                width='stretch',
                type="primary" if is_active else "secondary",
            ):
                st.session_state["enforcement_subpage"] = sub_name
                st.rerun()

    st.markdown("<div style='height:8px'></div>", unsafe_allow_html=True)
    sub_page = st.session_state["enforcement_subpage"]

    # ═══════════════════════════════════════════
    # SECTION 1 — ACCOUNT ENFORCEMENT
    # ═══════════════════════════════════════════
    if sub_page == "Account Enforcement":
        st.markdown("""
        <div class='finding-card' style='margin-bottom:18px;'>
            <div class='finding-title'>Content-level vs. account-level enforcement</div>
            <div class='finding-text'>
                Every other page in this dashboard scores individual reviews. This tab scores
                <b>accounts</b> — aggregating each reviewer's history into a single risk score,
                mapped to an enforcement action, the way a strikes system works. No single weak
                signal (e.g. merely posting on a high-volume day) can trigger an action alone —
                enforcement requires a genuine combination of evidence. See
                <code>analytics/account_risk_scoring.py</code> for the full formula.
            </div>
        </div>
        """, unsafe_allow_html=True)

        acct_df = load_csv("account_risk_scores.csv")
        if acct_df.empty:
            st.warning("reports/account_risk_scores.csv not found — run analytics/account_risk_scoring.py.")
        else:
            total_accounts = len(acct_df)
            action_counts = acct_df["Enforcement_Action"].value_counts()
            none_ct       = int(action_counts.get("None", 0))
            warn_ct       = int(action_counts.get("Warning", 0))
            restrict_ct   = int(action_counts.get("Restricted", 0))
            suspend_ct    = int(action_counts.get("Suspended", 0))

            c1, c2, c3, c4 = st.columns(4)
            kpi(c1, "Accounts Scored", f"{total_accounts}", "unique reviewer names", ACCENT)
            kpi(c2, "Clean", f"{none_ct}", f"{round(none_ct/total_accounts*100)}% of accounts", EMERALD)
            kpi(c3, "Warning", f"{warn_ct}", "flagged for analyst awareness", AMBER)
            kpi(c4, "Restricted / Suspended", f"{restrict_ct + suspend_ct}", "manual review or blocked", ROSE)

            col_a, col_b = st.columns([1, 1])
            with col_a:
                section("Enforcement Action Breakdown")
                fig = go.Figure(go.Bar(
                    x=["None", "Warning", "Restricted", "Suspended"],
                    y=[none_ct, warn_ct, restrict_ct, suspend_ct],
                    marker_color=[EMERALD, AMBER, ROSE, ROSE],
                    text=[none_ct, warn_ct, restrict_ct, suspend_ct],
            ...
            with col_b:
                section("Strike Ladder")
                st.markdown("""
                <div style='display:flex;flex-direction:column;gap:8px;margin-top:4px;'>
                    <div class='finding-card' style='border-left:3px solid #3DDC8C;'>
                        <div class='finding-title'>None — score < 2.0</div>
                        <div class='finding-text'>No action. Below the Warning threshold.</div>
                    </div>
                    <div class='finding-card' style='border-left:3px solid #F2B33D;'>
                        <div class='finding-title'>Warning — score ≥ 2.0</div>
                        <div class='finding-text'>Flagged for analyst awareness — no restriction applied.</div>
                    </div>
                    <div class='finding-card' style='border-left:3px solid #EF6F6F;'>
                        <div class='finding-title'>Restricted — score ≥ 4.0</div>
                        <div class='finding-text'>Future submissions routed to manual review before publishing.</div>
                    </div>
                    <div class='finding-card' style='border-left:3px solid #EF6F6F;'>
                        <div class='finding-title'>Suspended — score ≥ 6.0</div>
                        <div class='finding-text'>Account-level block — mirrors a repeated-violation strike ladder.</div>
                    </div>
                </div>
                """, unsafe_allow_html=True)

            section("Flagged Accounts (Warning tier and above)", tag=f"{warn_ct + restrict_ct + suspend_ct} accounts")
            flagged = acct_df[acct_df["Enforcement_Action"] != "None"].reset_index(drop=True)
            st.dataframe(flagged, width='stretch', hide_index=True)

    # ═══════════════════════════════════════════
    # TAB 2 — APPEALS & REINSTATEMENT
    # ═══════════════════════════════════════════
    elif sub_page == "Appeals & Reinstatement":
        st.markdown("""
        <div class='finding-card' style='margin-bottom:18px; border-left:3px solid #F2B33D;'>
            <div class='finding-title'>⚠ Modeled simulation, not real data</div>
            <div class='finding-text'>
                This dataset has no real appeals history. Appeal and overturn rates are
                documented assumptions defined in <code>config.py</code>
                (<code>APPEAL_RATE_ASSUMPTION</code>, <code>APPEAL_OVERTURN_RATE_ASSUMPTION</code>),
                sampled deterministically so results are reproducible. This demonstrates the
                <b>mechanism</b> — Detection → Enforcement → Appeal → Reinstatement — not a claimed
                real-world outcome.
            </div>
        </div>
        """, unsafe_allow_html=True)

        appeals_summary = load_csv("appeals_summary.csv")
        appeals_queue = load_csv("appeals_queue.csv")

        if appeals_summary.empty:
            st.warning("reports/appeals_summary.csv not found — run trust_safety/appeals_workflow.py.")
        else:
            s = appeals_summary.set_index("Metric")["Value"]
            c1, c2, c3, c4 = st.columns(4)
            kpi(c1, "Eligible (Critical/High)", f"{int(s['Eligible for Appeal (Critical/High)'])}", "cases eligible to appeal", ACCENT)
            kpi(c2, "Appealed (modeled)", f"{int(s['Appealed (modeled)'])}", f"{s['Appeal Rate Used (%)']}% appeal rate", AMBER)
            kpi(c3, "Overturned (modeled)", f"{int(s['Overturned on Appeal (modeled)'])}", f"{s['Overturn Rate Used (%)']}% overturn rate", EMERALD)
            kpi(c4, "Upheld (modeled)", f"{int(s['Upheld on Appeal (modeled)'])}", "enforcement action confirmed", ROSE)

            col_a, col_b = st.columns([1, 1])
            with col_a:
                section("Appeals Funnel")
                fig = go.Figure(go.Funnel(
                    y=["Eligible", "Appealed", "Overturned"],
                    x=[
                        int(s["Eligible for Appeal (Critical/High)"]),
                        int(s["Appealed (modeled)"]),
                        int(s["Overturned on Appeal (modeled)"]),
                    ],
                    marker=dict(color=[ACCENT, AMBER, EMERALD]),
            ...
            with col_b:
                section("Appeal Outcomes")
                fig2 = go.Figure(go.Pie(
                    labels=["Not Appealed", "Upheld", "Overturned"],
                    values=[
                        int(s["Eligible for Appeal (Critical/High)"]) - int(s["Appealed (modeled)"]),
                        int(s["Upheld on Appeal (modeled)"]),
                        int(s["Overturned on Appeal (modeled)"]),
                    ],
                    hole=0.6,
                    marker=dict(colors=[SLATE, ROSE, EMERALD], line=dict(color=INK, width=2)),
                    textfont=dict(color=TEXT_HI, size=12), textinfo='label+value',
                ))
                chart_layout(fig2, 320)
                fig2.update_layout(showlegend=False)
                st.plotly_chart(fig2, width='stretch')

            if not appeals_queue.empty:
                section("Appealed Cases", tag=f"{int(s['Appealed (modeled)'])} cases")
                appealed_only = appeals_queue[appeals_queue["Appeal_Status"] == "Appealed"].drop(columns=["Data_Note"], errors="ignore")
                st.dataframe(appealed_only.reset_index(drop=True), width='stretch', hide_index=True)

    # ═══════════════════════════════════════════
    # TAB 3 — ABUSE VECTOR TAXONOMY
    # ═══════════════════════════════════════════
    elif sub_page == "Abuse Vector Taxonomy":
        st.markdown("""
        <div class='finding-card' style='margin-bottom:18px;'>
            <div class='finding-title'>Detect volume, then separately assess intent</div>
            <div class='finding-text'>
                A burst detector's job is to flag volume anomalies — that is not the same signal
                as malicious intent. Every flagged signal below required a second pass before
                triage. Full writeup in <code>ABUSE_VECTORS.md</code>.
            </div>
        </div>
        """, unsafe_allow_html=True)

        burst_df = load_csv("review_burst_detection.csv")
        dup_summary = load_csv("duplicate_review_summary.csv")
        acct_df2 = load_csv("account_risk_scores.csv")

        n_burst_days = int(burst_df["Burst_Detected"].sum()) if not burst_df.empty and "Burst_Detected" in burst_df.columns else 7
        n_dup_flags = int(dup_summary["Flags_Found"].sum()) if not dup_summary.empty else 0
        n_flagged_accounts = int((acct_df2["Enforcement_Action"] != "None").sum()) if not acct_df2.empty else 0
        n_total_accounts = len(acct_df2) if not acct_df2.empty else 0

        vector_rows = [
            ("Coordinated volume (brigading)", "Burst detection (static + rolling)", f"{n_burst_days} / 245 days", "All benign on review — sentiment-checked"),
            ("Sockpuppet / multi-account", "Suspicious reviewer + account risk scoring", f"{n_flagged_accounts} / {n_total_accounts} accounts (Warning tier)", "No Restricted/Suspended-tier accounts found"),
            ("Duplicated / fake content", "Exact + fuzzy + same-day match", f"{n_dup_flags} / 300 reviews", "Validated negative (detector proven functional)"),
            ("Genuine policy complaints", "Severity/priority pipeline", "145 / 300 (Critical+High)", "Routed as content policy, not abuse"),
        ]
        vector_df = pd.DataFrame(vector_rows, columns=["Vector", "Detector", "Flagged", "Verdict"])
        section("Abuse Vector Summary")
        st.dataframe(vector_df, width='stretch', hide_index=True)

        section("Known Gap — Account Acquisition")
        finding(
            "Not demonstrable on this dataset",
            "Account-acquisition methods can't be assessed here — this dataset has no "
            "signup flow, device ID, IP, or account-age field — so account-acquisition-vector "
            "analysis genuinely can't be demonstrated here. Documented honestly in ABUSE_VECTORS.md "
            "rather than filled in with a plausible-sounding guess."
        )

    # ═══════════════════════════════════════════
    # TAB 4 — OPS CAPACITY
    # ═══════════════════════════════════════════
    elif sub_page == "Ops Capacity":
        ops_df = load_csv("ops_capacity.csv")
        if ops_df.empty:
            st.warning("reports/ops_capacity.csv not found — run analytics/ops_capacity_analysis.py.")
        else:
            o = ops_df.set_index("Metric")["Value"]

            st.markdown("""
            <div class='finding-card' style='margin-bottom:18px;'>
                <div class='finding-title'>Two different questions, kept separate on purpose</div>
                <div class='finding-text'>
                    Steady-state load (real, from actual Review_Date span) and SLA-driven burst
                    capacity (modeled handling-time assumptions) answer different questions.
                    Blending them into one "we need X analysts" number would misrepresent both.
                </div>
            </div>
            """, unsafe_allow_html=True)

            section("Steady-State — Real Arrival Rate")
            c1, c2, c3, c4 = st.columns(4)
            kpi(c1, "Real Avg Reviews/Week", f"{o['[Arrival] Real Avg Reviews/Week']}",
                f"over {o['[Arrival] Observed Span (weeks)']} weeks", ACCENT)
            kpi(c2, "Weekly Load (modeled hrs)", f"{o['[Steady-State] Modeled Weekly Handling Load (hours)']}",
                "handling time at current mix", CYAN)
            one_analyst_cap = o["[Steady-State] One Analyst's Weekly Capacity (hours)"]
            kpi(c3, "1 Analyst Capacity", f"{one_analyst_cap}h/wk",
                "7.5h × 5 days", SLATE)
            kpi(c4, "Utilization", f"{o['[Steady-State] Utilization of 1 Analyst (%)']}%",
                f"{int(o['[Steady-State] Analysts Needed at Real Arrival Rate'])} analyst(s) needed", EMERALD)

            section("SLA Burst — Clearing the Current Backlog Snapshot")
            c5, c6, c7 = st.columns(3)
            kpi(c5, f"Critical SLA ({SLA_P1_HOURS}h)", f"{int(o['[SLA Burst] Analysts Needed to Clear Critical Backlog Within SLA (4h)'])}",
                f"{o['[SLA Burst] Critical Backlog Total Handling Time (hours)']}h of work", ROSE)
            kpi(c6, f"High SLA ({SLA_P2_HOURS}h)", f"{int(o['[SLA Burst] Analysts Needed to Clear High Backlog Within SLA (24h)'])}",
                f"{o['[SLA Burst] High Backlog Total Handling Time (hours)']}h of work", AMBER)
            kpi(c7, "Peak Concurrent", f"{int(o['[SLA Burst] Peak Concurrent Analysts Needed (Critical + High overlap)'])}",
                "analysts needed at once, worst case", VIOLET)

            section("Volume by Severity Tier")
            vol_rows = []
            for tier in ["Critical", "High", "Medium", "Low", "Safe"]:
                key = f"[Volume] {tier} Cases"
                if key in o.index:
                    vol_rows.append((tier, int(o[key])))
            vol_df = pd.DataFrame(vol_rows, columns=["Tier", "Count"])
            fig3 = go.Figure(go.Bar(
                x=vol_df["Tier"], y=vol_df["Count"],
                marker_color=[ROSE, AMBER, VIOLET, CYAN, EMERALD][:len(vol_df)],
                text=vol_df["Count"], textposition="outside",
            ))
            chart_layout(fig3, 300)
            st.plotly_chart(fig3, width='stretch')

    # ═══════════════════════════════════════════
    # TAB 5 — REAL-TIME DETECTION SIMULATOR
    # ═══════════════════════════════════════════
    elif sub_page == "Real-Time Simulator":
        st.markdown("""
        <div class='finding-card' style='margin-bottom:18px;'>
            <div class='finding-title'>Batch → streaming, demonstrated rather than just described</div>
            <div class='finding-text'>
                Every pipeline in this project runs as a periodic batch job against a static
                SQLite file — documented as a platform-scale limitation elsewhere in this
                dashboard. This tab streams real reviews from the database one at a time, scoring
                each on arrival, to show what the streaming version of the same logic looks like.
                The scoring logic itself is unchanged — same SEVERITY_RULES from config.py.
            </div>
        </div>
        """, unsafe_allow_html=True)

        sim_n = st.slider("Reviews to stream", min_value=5, max_value=40, value=15, key="sim_n")
        run_sim = st.button("▶  Start streaming simulation", key="run_live_sim")

        if run_sim:
            sim_reviews = load_db(f"SELECT * FROM Reviews ORDER BY RANDOM() LIMIT {sim_n}")
            sim_reviews["Risk_Level"] = sim_reviews["Label"].map(RISK_MAP)

            live_metric_slot = st.empty()
            live_table_slot = st.empty()
            progress = st.progress(0)

            streamed_rows = []
            counts = {"Safe": 0, "Needs Review": 0, "High Risk": 0}

            import time
            for i, (_, row) in enumerate(sim_reviews.iterrows()):
                ...
                with live_metric_slot.container():
                    m1, m2, m3, m4 = st.columns(4)
                    kpi(m1, "Ingested", f"{i+1}/{sim_n}", "reviews streamed", ACCENT)
                    kpi(m2, "Safe", f"{counts.get('Safe', 0)}", "auto-cleared", EMERALD)
                    kpi(m3, "Needs Review", f"{counts.get('Needs Review', 0)}", "queued", AMBER)
                    kpi(m4, "High Risk", f"{counts.get('High Risk', 0)}", "P1/P2 routed", ROSE)

                live_table_slot.dataframe(
                    pd.DataFrame(streamed_rows).iloc[::-1], width='stretch', hide_index=True
                )
                progress.progress((i + 1) / sim_n)
                time.sleep(0.15)

            st.success(f"Streamed {sim_n} reviews · same SEVERITY_RULES/RISK_MAP as the batch pipeline, applied per-item on arrival.")
    ...
    if not groq_setup_error:
        ...
        with st.container():
            ...
            for idx, (icon, q) in enumerate(quick_questions):
                with quick_cols[idx]:
                    if st.button(f"{icon}\n{q}", key=f"quick_q_{idx}", width='stretch'):
        ...
        with st.container():
            ...
            for col_idx, (icon, q) in enumerate(dental_full):
                with cols_dental[col_idx]:
                    if st.button(f"{icon}\n{q}", key=f"dental_q_{col_idx}", width='stretch', help=q):
                        with st.spinner("PraxisIQ Copilot is thinking..."):
                            ask_copilot(q)
                        st.rerun()
        ...
        with st.container():
            ...
            project_full = [
                ...
                ("🌐", "What would need to change about this pipeline to work at platform scale?"),
            ]
            cols_project = st.columns(5, gap="small")
            for col_idx, (icon, q) in enumerate(project_full):
                with cols_project[col_idx]:
                    if st.button(f"{icon}\n{q}", key=f"project_q_{col_idx}", width='stretch', help=q):
                        with st.spinner("PraxisIQ Copilot is thinking..."):
                            ask_copilot(q)
                        st.rerun()
        ...
        with st.container():
            ...
            for col_idx, (icon, q) in enumerate(web_full):
                with cols_web[col_idx]:
                    if st.button(f"{icon}\n{q}", key=f"web_q_{col_idx}", width='stretch', help=q):
                        with st.spinner("PraxisIQ Copilot is thinking..."):
                            ask_copilot(q)
                        st.rerun()
